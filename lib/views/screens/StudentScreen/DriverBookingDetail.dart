...
class DriverBookingDetail extends StatefulWidget {
  const DriverBookingDetail({super.key});
...
// Fare Range Guideline Widget
  ...
  Widget build(BuildContext context) {
    final bookingStream = userBookings.isNotEmpty
        ? getBookingStream(userBookings.first['id'])
        : null;
    print(userBookings);
    print(bookingStream);
...
// Keep the existing _buildDetailRow method

// Placeholder method - implement according to your app's requirements
  void _launchDialer(String phoneNumber) {
    // Implement phone dialer functionality
    // You might use a package like url_launcher
  }
  ...
  Widget _buildBookingCard(
    ...
    return StreamBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        // Error State
        if (snapshot.hasError) {
          return _buildErrorCard(snapshot.error, screenWidth);
        }

        // No Data State
        if (!snapshot.hasData || snapshot.data == null) {
          return _buildNoDataCard(screenWidth);
        }
...
// Show booking options bottom sheet
  ...
  Widget _buildErrorCard(Object? error, double screenWidth) {
    return Card(
      margin: EdgeInsets.symmetric(vertical: screenWidth * 0.03),
      color: Colors.red[50],
      child: Padding(
  ...
  Widget _buildNoDataCard(double screenWidth) {
    return Card(
      margin: EdgeInsets.symmetric(vertical: screenWidth * 0.03),
      child: const Padding(
        padding: EdgeInsets.all(16.0),
